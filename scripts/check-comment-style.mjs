...
function collectCommentBlockViolations(filePath, text, output) {
  const lines = text.split(/\r?\n/);
  let consecutive = 0;
    ...
    if (/^\s*\/\//.test(lines[index])) {
      ...
      if (consecutive === 4) {
        output.push({
          ...
          detail: 'max 3 consecutive // lines',
        });
      }
      continue;
    }
...
function scanFile(rootDir, relativePath) {
  ...
  collectCommentBlockViolations(relativePath, text, violations);
...
try {
  ...
  if (violations.length > 0) {
    ...
    console.error(
      'Rules: require file headers, max 3 // lines, no block or inline comments, use &/w/, no trailing punctuation'
    );
    console.error('');
  ...
  console.log(
    `Comment style check passed (${uniqueFiles.length} file(s) scanned)`
  );
} catch (error) {
